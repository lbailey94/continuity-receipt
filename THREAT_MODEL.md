...
| 7 | **Key compromise** | Partially handled (0.2) | Bundle-level self-signed revocation statements; receipts issued **at or after** `revoked_at` fail (`key_revoked`). Receipts issued **before** revocation remain valid. **Open gap:** compromise *before* the revocation is published is indistinguishable — anchoring and monitoring are the outer-layer remedies. No distributed CRL exists. |
