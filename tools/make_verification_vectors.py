...
Reproducible structure: fixed keys (`verifier-1`), fixed `verified_at`, fixed
bundle task ids. The bundle fixtures are generated through
...
IMPLEMENTATION_VERSION = "0.3.2"  # pinned in vectors; bump with the release


def write(name: str, document: dict) -> str:
    path = VECTORS / name
...
def unsigned_base(bundle: dict) -> dict:
    return {
...
def main() -> int:
    ...
    valid = verification.issue_verification_receipt(
        bundle,
        "TRUSTED",
        [],
        issuer=VERIFIER_DID,
    ...
    record("01_valid.json", True, [], "valid receipt; digest matches bundle.json",
    ...
    record("03_tampered_verdict.json", False, ["bad_signature"],
           "verdict changed after signing")
    ...
    bad_verdict = unsigned_base(bundle)
    ...
    bad_digest = unsigned_base(bundle)
    ...
    extra_member = unsigned_base(bundle)
    ...
    bad_codes = unsigned_base(bundle)
